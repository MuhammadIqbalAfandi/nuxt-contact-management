...
const handleSubmitUpdateProfile = async () => {
  try {
    const response = await userUpdateProfile(token.value, user.name);

    await alertSuccess("Profile updated successfully");
    user.name = response.data.name; // Update local state with new name
  } catch (e) {
    const error = e as FetchError;
    await alertError(error.data.errors || "Failed to update profile");
  }
};
...
const handleSubmitUpdatePassword = async () => {
  ...
  try {
    await userUpdatePassword(token.value, user.password);

    await alertSuccess("Password updated successfully!");
    user.password = ""; // Clear password fields after successful update
    user.confirmPassword = "";
  } catch (e) {
    const error = e as FetchError;
    await alertError(error.data.errors || "Failed to update password");
  }
};
...
onBeforeMount(async () => {
  try {
    ...
    const error = e as FetchError;
    console.error("Error fetching user details:", error.data.errors);
  }
});
</script>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
    ...
    >
      <div class="p-6">
        ...
        <form @submit.prevent="handleSubmitUpdatePassword">
          <div class="mb-5">
            <label
              for="new_password"
              class="block text-gray-300 text-sm font-medium mb-2"
