...
import { contactDelete, contactDetails } from "@lib/api/ContactApi";
...
const fetchContacts = async () => {
  try {
    ...
    const response = await contactDetails(token.value, payload);
    contacts.value = response.data;
    currentPage.value = response.paging.current_page;
    totalPage.value = response.paging.total_page;
  } catch (e) {
    const error = e as FetchError;
    console.log(error.data.errors || "Error fetching contacts");
  }
};
...
const handleDeleteContact = async (contactId: number) => {
  try {
    ...
    await contactDelete(token.value, contactId);
    ...
    const error = e as FetchError;
    await alertError(
      error.data.errors || "Failed to delete contact. Please try again later."
    );
  }
};
...
<template>
  ...
  >
    ...
    <div id="searchFormContent" class="mt-4">
      <form @submit.prevent="handleSearchContacts">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    >
      <div class="p-6">
        <NuxtLink
          :to="{
            name: 'dashboard-contacts-contactId-view',
