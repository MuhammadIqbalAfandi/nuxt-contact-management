...
import { addressDelete, addressDetails } from "@lib/api/AddressApi";
...
const fetchAddresses = async () => {
  try {
    const response = await addressDetails(token.value, contactId);
    listAddress.value = response.data;
  } catch (e) {
    console.log("Failed to load address detail");
  }
};
...
const handleDeleteAddress = async (addressId: number) => {
  try {
    ...
    await addressDelete(token.value, contactId, addressId);
    await alertSuccess("Address deleted successfully");
    fetchAddresses();
  } catch (e) {
    await alertError("Failed to delete address. Please try again later.");
  }
};
  ...
  >
    <div class="p-8">
      ...
      <div class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          >
            <NuxtLink
              :to="{
                name: 'dashboard-contacts-contactId-addresses-create',
                params: { contactId },
              }"
              class="block h-full"
            >
          ...
          >
            ...
            <div class="flex justify-end space-x-3">
              <NuxtLink
                :to="{
                  name: 'dashboard-contacts-contactId-addresses-addressId-edit',
                  params: { contactId, addressId: address.id },
                }"
      ...
      <div class="flex justify-end space-x-4">
        ...
        <NuxtLink
          :to="{
            name: 'dashboard-contacts-contactId-edit',
            params: { contactId },
          }"
